...
/*layer parameter added to handle layers with xover between*/
struct cost *insert(double min_cost,int row,int col, int layer)
{
...
/*  	new_cell = (struct cost *)(G_malloc(sizeof(struct cost))); */
	...
	new_cell->row = row;
	new_cell->col = col;
        /*DKS assign layer for xover mode: base layer(0) or overlay(1)*/
        new_cell->layer = layer;
	new_cell->above = NULL ;
...
struct cost *find(double min_cost,int row,int col, int layer)
{
...
int show(struct cost *next)
{
	...
	for (next_cell=next;next_cell!=NULL;next_cell=next_cell->nexttie)
		fprintf(stderr, "%p %d,%d,%f %p %p %p %p %p\n",
			next_cell,
			next_cell->row,
			next_cell->col,
			next_cell->min_cost,
			next_cell->nexttie,
			next_cell->previoustie,
			next_cell->lower,
			next_cell->higher,
			next_cell->above) ;
	show(next->lower) ;
